...
module.exports = {
  ...
  async execute(message, args) {
    ...
    while (games.length < numberOfGames) {
      const { data: game, error } = await getGame(filter, Math.ceil(Math.random() * 5000));
      if (error) return errorHandler(message, error);
      games.push(game);
    }
...
const getCover = async (coverID) => {
  return fetch(`${gameAPIURL}/covers`, {
    ...
    .then(([json]) => {
      ...
      return { data: json.url.slice(2, json.url.length).replace('t_thumb', 't_cover_big') };
    })
    .catch((error) => {
      return { error };
    });
...
const parseArgs = (args) => {
  ...
  try {
    args.forEach((arg) => {
      ...
      if (parameter === 'number') {
        if (2 > input > 4) throw 'Due to rate limiting, please enter a number between 2 and 4 for the number of games.';
